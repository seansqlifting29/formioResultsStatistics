...
function formioStats(arr) {
    var c = 0;                     // Establish count for true (c) and false (d)
    var d = 0;
    for (let i in arr) {           // for loop (let i in each of the objects in the array (arr) input into our function; start i = 0, i < length of arr, and i increment by 1 for:
        for (let i2 in arr[i].data) {     // for loop (let i2 represent the attributes mapping to a boolean value from the data attribute inside each of the objects within each of the objects of arr; input into our function; start i2 = 0, i2 < length of object belonging to data attribute, and i increment by 1)
...
const b = formioStats(formioResults);
